...
Record: which areas the PR touches (`src/notes/`, `src/pageAddress/`, `src/background/`, `src/ui/`, `noteDialog/`, `notesPage/`, `test/`, `adr/`, `scripts/`, `.github/`, `manifest.json`), the total scope (additions/deletions, file count), the stated intent (commits + PR description), and any implementation proposal from the issue discussion.
...
- **pattern-scout** (only when the diff adds a new module, stored field, page section, or export format): find how similar ones are built here and report the convention with real examples.
