...
Do **not** fetch "the newest N issues": `gh issue list --limit N` returns only the most recently created, so an older duplicate is never seen. Search by **relevance across the whole repo**, both states. Scope every `gh search issues` to this repo with `--repo <owner>/<repo>` (without it, `gh search` queries all of GitHub).
...
2. **Search each set, relevance-ranked** (put the keywords BEFORE any qualifier, or `gh search` returns an empty list):
   ```bash
   gh search issues --repo <owner>/<repo> "<keywords> in:title" --limit 20 --json number,title,state,url   # title-scoped: strongest signal
   gh search issues --repo <owner>/<repo> "<keywords>" --limit 30 --json number,title,state,url            # title + body: differently worded duplicates
   ```
...
When area labels apply, extend the `--label` value comma-separated (for example `--label "recipes,waiting-for-human-check"`); with none, keep it exactly as above. Never emit a leading or trailing comma.
