...
- **The menu item id and the command name `add-note-to-selection` are Chrome state.** A changed menu id leaves the old item in place until the next install or update. A changed command name drops the user's own shortcut key.
